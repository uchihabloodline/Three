...
function findAngle(p0x,p0y,p1x,p1y,p2x,p2y) {   //to fangle
...
function initialiseSceneVariables() {   //iniSceneVars
  ...
  XminD=-10;
  XmaxD=10;
 // mirror distance adjustment
  centerXM=myCenterX+2.5;
  centerYM=myCenterY+0.5;
  mirrorL=0.2;
  centerYMD=centerYM-0.3;
  centerYMU=centerYM+0.3;
...
//function for ships add.
function textBox(text){



  var geometryA = new THREE.BoxGeometry( mySceneW/15, mySceneH/10, 0);
  var materialA = new THREE.MeshBasicMaterial({map: image,transparent:true ,color: 0xffff00} );
  var adder = new THREE.Mesh( geometryA, materialA );
  return adder;
...
function setFire(){
  ...
  if(l1 && l1U && l1D)fireBox.position.set(b.position.x, b.position.y, 1);
  else if(l2 && l2U && l2D)fireBox.position.set(c.position.x, c.position.y, 1);
  else if(l3 && l3U && l3D)fireBox.position.set(d.position.x, d.position.y, 1);
...
function changeAngleMirrorD(num){ //angmirrorD
  t1=num;
  var th=(Math.PI/180)*2;
  var ang=th*num;
  var newX=centerXM+(mirrorL/4 )*(Math.sin(ang));
  var newY=centerYMD+(mirrorL/4)*(Math.cos(ang));
  var newX2=centerXM-(mirrorL/4)*(Math.sin(ang));;
  var newY2=centerYMD-(mirrorL/4)*(Math.cos(ang));
...
var helpContent;  //will do this later..
function initialiseHelp(){
  helpContent="<h2>Reflection through mirror</h2>";
  helpContent=helpContent+"<p>drag the slider in input panel or drag the mirror to change the angle of mirrors.</p>";
  helpContent=helpContent+"<p>If all three ray of light will fall on ship then the ship will get destroyed with catching of fire</p>"
  helpContent=helpContent+"<p>To see how ray are reflected,press start to start the animation.</p>"
...
var infoContent;  //will do this later
function initialiseInfo(){
  infoContent ="<h2>Reflection of rays at mirror</h2>";
  infoContent =infoContent+"<p> Greek Scientist Archimedes used a giant mirror, those mirror were used to set fire on roman ships.Those ships were attacking his home city of Syracuse.</p>";
...
function loadExperimentElements() {
  ...
  PIEsetDeveloperName("Shivam Pandey");
  ...
  PIEaddInputSlider("1st Mirror", Xdefault, handleU, XminU, XmaxU, Xstep);
  PIEaddInputSlider("2nd Mirror", Xdefault, handleX, Xmin, Xmax, Xstep);
  PIEaddInputSlider("3rd Mirror", Xdefault, handleD, XminD, XmaxD, Xstep);

// image/object of fire has been loaded from here
  fire = THREE.ImageUtils.loadTexture('ball.png', {}, function() {
  PIErender();});
...
// pirate ships image is loaded from here.
  ...
  var texture = THREE.ImageUtils.loadTexture('ball.png', {}, function() {
    myBackp.position.set(myCenterX/2-0.12, myCenterY/2-0.05, -0.09);
  PIErender();});
...
//for the transmitting and reflecting rays
  line1=drawMatchStick(myCenterX/2, myCenterY/2,myCenterX+1,centerYM,'#ff0000',ray);
  mirror=drawMatchStick(centerXM, centerYM+mirrorL/2,centerXM,centerYM-mirrorL/2,"#ff0000",10);
  ...
  line1U=drawMatchStick(myCenterX/2, myCenterY/2,centerXM,centerYMU,'#ff0000',ray);
  mirrorU=drawMatchStick(centerXM, centerYMU+mirrorL/2,centerXM,centerYMU-mirrorL/2,"#ff0000",10);
  ...
  line1D=drawMatchStick(myCenterX/2, myCenterY/2,centerXM,centerYMD,'#ff0000',ray);
  mirrorD=drawMatchStick(centerXM, centerYMD+mirrorL/2,centerXM,centerYMD-mirrorL/2,"#ff0000",10);
...
function resetExperiment(){
   ...
   PIEchangeInputSlider("1st Mirror",Xdefault);
   PIEchangeInputSlider("2nd Mirror",Xdefault);
   PIEchangeInputSlider("3rd Mirror",Xdefault);
   PIErender();
   l2D=true;
